...
import  {Element}  from '@/interfaces/app.interfaces'
...
const Modal = () => {
  const { modal, setModal, currentMovi } = useInfoStore();
  ...
  const api_key = process.env.NEXT_PUBLIC_API_KEY as string;
	...
	const api = `${base_url}/${currentMovi?.media_type === 'tv' ? 'tv' : 'movie'}/${
		currentMovi.id
				...
				<div className='flex space-x-16 rounded-b-md bg-[#181818] px-10 py-8'>
					<div className='space-y-6 text-lg'>
						<div className='flex items-center space-x-2 text-sm'>
							<p className='font-semibold text-green-400'>{currentMovi!.vote_average * 10}% Match</p>
							<p className='font-light'>{currentMovi?.release_date}</p>
							<div className='flex h-4 items-center justify-center rounded border border-white/40 px-1.5 text-xs'>HD</div>
						</div>

						<div className='flex flex-col gap-x-10 gap-y-4 font-light md:flex-row'>
							<p className='w-5/6'>{currentMovi?.overview}</p>
							<div className='flex flex-col space-y-3 text-sm'>
								<div>
									<span className='text-[gray]'>Original language:</span> {currentMovi?.original_language}
								</div>

								<div>
									<span className='text-[gray]'>Total votes:</span> {currentMovi?.vote_count}
								</div>
							</div>
						</div>
					</div>
