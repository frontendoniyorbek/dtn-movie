...
const Hero = ({trending}: HeroProps): JSX.Element => {
  ...
  return (
    <div className='flex flex-col space-y-2 py-20 md:space-y-4 lg:h-[65vh] lg:pb-12 lg:center'>
      ...
      <h1 className='text-2xl font-bold md:text-4xl lg:text-7xl'>{movie?.title || movie?.original_name}</h1>
      <p className='max-w-xs md:max-w-lg lg:max-w-2xl text-xs text-shadow-md md:text-lg lg:text-2xl'>{movie?.overview?.slice(0, 100)}...</p>

      <div>
        <button onClick={handleCurrentMovie} className='flex justify-center items-center space-x-2 bg-white/60 hover:bg-white/80 transition-all font-bold text-black w-[200px] h-[56px] rounded-full'><TbPlayerPlay className='h-5 w-5md:h-8 w-8'/> Watch now</button>
      </div>
    </div>
  )
}
